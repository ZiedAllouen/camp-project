...
export const getItemsBySearch = async (req, res) => {
    ...
    try {
        ...
        const items = await Item.find({ $or: [ { item }, { tags: { $in: tags.split(',') } } ]});
...
export const  updateItem = async (req, res) => {
    const { id } = req.params;
    const { item,address ,phone, message, selectedFile, creator, tags } = req.body;
    
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`No post with id: ${id}`);

    const updatedItem = { item,address ,phone, message, selectedFile, creator, tags, _id: id };
